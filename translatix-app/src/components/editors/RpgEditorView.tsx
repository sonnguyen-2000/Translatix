...
import { RpgFileData } from '@/data/mockRpgFiles';
...
export default function RpgEditorView({ gameName, files, onGoBack }: RpgEditorViewProps) {
  return (
      ...
      <div className="flex flex-grow flex-col min-h-0 p-4 md:p-6">
        ...
        <div className="flex-shrink-0 flex items-center justify-between mb-4 space-x-4">
          <div className="flex items-center space-x-2">
            <button
              onClick={onGoBack}
              className="text-sm font-semibold py-2 px-4 rounded-md shadow-sm bg-surface-2 border border-default text-primary hover:bg-hover flex items-center space-x-2"
            >
              <ArrowLeft className="h-4 w-4" />
              <span>Chọn Game Khác</span>
            </button>
            <button
              className="bg-gray-400 text-white text-sm font-semibold py-2 px-4 rounded-md shadow-lg flex items-center space-x-2 cursor-not-allowed"
              disabled
            >
        ...
        <div className="flex-grow grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-0">
            ...
            <div className="flex-grow overflow-y-auto pr-2">
              <ul className="space-y-1">
                {files.map(file => (
                  <li
                    key={file.name}
                    className={`tree-item flex items-center p-2 rounded-md cursor-pointer ${file.selected ? 'active' : ''}`}
                  >
                    <input
                      type="checkbox"
                      defaultChecked={file.checked}
                      className="mr-3 h-4 w-4 rounded border-default text-indigo-600 bg-surface-2 focus:ring-indigo-500"
                    />
                    <FileJson className="h-4 w-4 mr-2 text-secondary icon flex-shrink-0" />
                    <span className="flex-grow truncate text-sm text-primary">{file.name}</span>
                    <span className="text-xs font-mono ml-2 text-secondary">{file.progress}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="lg:col-span-4 flex flex-col rounded-lg p-3 min-h-0 bg-surface-1 border border-default card">
            ...
            <div className="flex-grow rounded-md p-3 text-base leading-relaxed overflow-y-auto space-y-2 bg-surface-2">
              <p className="text-primary">Diễn viên</p>
              <div className="my-1 border-t border-default"></div>
              <p className="text-primary">Nhân vật chính</p>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-col rounded-lg p-3 min-h-0 bg-surface-1 border border-default card">
            ...
            <div className="flex-grow rounded-md p-3 text-base leading-relaxed overflow-y-auto space-y-2 bg-surface-2">
              <p className="text-primary">Actor</p>
              <div className="my-1 border-t border-default"></div>
              <p className="text-primary">Main Character</p>
            </div>
          </div>
        </div>
      </div>
